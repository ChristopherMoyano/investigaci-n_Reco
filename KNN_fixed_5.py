...
tag_df = "vuln_2020_2025_num+texto"
...
DRY_RUN = False   # <-- deja True para tu primera corrida con num_cluster=16, luego cambia a False
...
if DRY_RUN:
    ...
    param_list = param_list[:3]   # incluye idealmente al menos una combinacion con p=1 (la mas cara)
    num_exp = 3
else:
    num_exp = 50

# num_cluster = numero de combinaciones -> cada una arranca de inmediato,
# ninguna espera en cola a que se libere un engine. No cambia ningun
# resultado, solo elimina el tiempo perdido esperando turno.
num_cluster = len(param_list)

os.makedirs("pickle", exist_ok=True)
os.makedirs("pickle/checkpoints", exist_ok=True)
os.makedirs("pickle/best_models", exist_ok=True)   # <- carpeta dedicada a los mejores modelos
...
def asignaciones(params, num_exp, checkpoint_every, tag_df, tag_num_tex, tag_modelo):
    ...
    for i in range(num_exp):
        try:
            ...
            # working_memory acota el tamano de los bloques internos que usa
            # sklearn para calcular distancias por fuerza bruta. No cambia el
            # resultado, solo evita picos de memoria al correr 16 engines a la vez.
            ...
            # recall de la clase positiva (has_kev=1) sobre el set de test
            ...
            # --- checkpoint LIVIANO: solo metricas, nunca el modelo completo ---
    ...
    # --- guardar el mejor modelo DIRECTO A DISCO, aqui mismo en el engine ---
    # asi nunca viaja el objeto pesado de vuelta al proceso principal por red/IPC,
    # y nunca se acumulan 16 modelos pesados en RAM del proceso principal a la vez.
    ...
    # Se evita "del" sobre una lista de variables: si el loop de num_exp fallo
    # en TODAS sus iteraciones antes de asignar X_train/X_test/y_train/y_test
    # (por ejemplo si confusion_matrix devuelve una matriz mas chica de 2x2
    # por falta de alguna clase en un split), "del" lanza UnboundLocalError
    # sobre la primera variable no asignada que encuentra. Reasignar a None
    # es seguro siempre, porque X, y y mejor_modelo estan garantizados como
    # definidos en cualquier ejecucion que llegue hasta aqui.
    X = None
    y = None
    mejor_modelo = None
    gc.collect()
    ...
    # el retorno YA NO incluye el modelo pesado, solo metadatos livianos
    return {
        "n_neighbors": n_neighbors_v,
        "weights": weights_v,
        "p": p_v,
...
# resultado liviano (metricas + rutas a los mejores modelos, no los modelos en si)
with open(f"pickle/{tag_modelo}_par_results_{tag_df}_{tag_num_tex}.pckl", "wb") as f:
    pickle.dump(results, f, protocol=pickle.HIGHEST_PROTOCOL)

all_conf_train = []
all_conf_test = []
tabla_mejores = []   # resumen legible: que combinacion gano, y donde esta guardado su modelo
...
# ------------------------------------------------------------------
# COMO CARGAR UN MEJOR MODELO DESPUES, PARA PROBARLO CON OTRA BASE:
#
#   import pickle
#   with open("pickle/best_models/best_KNN_n11_wdistance_p2_vuln_2020_2025_num+texto.pckl", "rb") as f:
#       data = pickle.load(f)
#   modelo = data["modelo"]
#   y_pred_nuevo = modelo.predict(X_nuevo)   # X_nuevo debe tener las MISMAS columnas/orden
# ------------------------------------------------------------------